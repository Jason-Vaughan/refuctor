...
// import AfterDarkMode from './components/AfterDarkMode';
...
const App = () => {
  ...
  // REMOVED: Frontend debt calculation functions - now using SSOT backend data
  // getDebtCounts, getDetailedDebtBreakdown, getTopHotspots eliminated per anti-bloat rules
  ...
  return (
    <div className="dashboard">
          ...
          <div className="modal-overlay" onClick={closeDebtModal}>
            <div className="debt-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-content">
                {/* Summary Stats - SSOT Data */}
                <div className="debt-summary-stats">
                  <div className="summary-stat">
                    <div className="stat-value">{financialMetrics?.debtStatus?.summary?.total || 0}</div>
                    <div className="stat-label">Total Issues</div>
                  </div>
                  {(financialMetrics?.debtStatus?.currentDebt?.Guido?.length || 0) > 0 && (
                    <div className="summary-stat guido">
                      <div className="stat-value">{financialMetrics?.debtStatus?.currentDebt?.Guido?.length || 0}</div>
                      <div className="stat-label">🤌 Guido Categories</div>
                    </div>
                  )}
                  <div className="summary-stat">
                    <div className="stat-value">{financialMetrics?.debtStatus?.topHotspots?.length || 0}</div>
                    <div className="stat-label">Hotspot Files</div>
                  </div>
                </div>

                {/* Debt Breakdown by Category - SSOT Data */}
                <div className="debt-breakdown-section">
                  <h3>📋 Issues by Category</h3>
                  <div className="debt-categories">
                    {financialMetrics?.debtStatus?.summary?.markdown > 0 && (
                      <div className="debt-category-card high">
                        <div className="category-header">
                          <span className="category-icon">📝</span>
                          <div className="category-info">
                            <h4>Markdown Issues</h4>
                            <p>Documentation formatting and structure issues</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.markdown}</div>
                        </div>
                      </div>
                    )}
                    {financialMetrics?.debtStatus?.summary?.spelling > 0 && (
                      <div className="debt-category-card medium">
                        <div className="category-header">
                          <span className="category-icon">📚</span>
                          <div className="category-info">
                            <h4>Spelling Errors</h4>
                            <p>Misspelled words and typos in documentation</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.spelling}</div>
                        </div>
                      </div>
                    )}
                    {financialMetrics?.debtStatus?.summary?.codeQuality > 0 && (
                      <div className="debt-category-card low">
                        <div className="category-header">
                          <span className="category-icon">💻</span>
                          <div className="category-info">
                            <h4>Code Quality Issues</h4>
                            <p>Console.log statements, TODOs, and code smells</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.codeQuality}</div>
                        </div>
                      </div>
                    )}
                    {financialMetrics?.debtStatus?.summary?.security > 0 && (
                      <div className="debt-category-card critical">
                        <div className="category-header">
                          <span className="category-icon">🔒</span>
                          <div className="category-info">
                            <h4>Security Vulnerabilities</h4>
                            <p>Known security issues in dependencies</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.security}</div>
                        </div>
                      </div>
                    )}
                    {financialMetrics?.debtStatus?.summary?.eslint > 0 && (
                      <div className="debt-category-card medium">
                        <div className="category-header">
                          <span className="category-icon">🔧</span>
                          <div className="category-info">
                            <h4>ESLint Issues</h4>
                            <p>Code style and quality violations</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.eslint}</div>
                        </div>
                      </div>
                    )}
                    {financialMetrics?.debtStatus?.summary?.typescript > 0 && (
                      <div className="debt-category-card high">
                        <div className="category-header">
                          <span className="category-icon">📘</span>
                          <div className="category-info">
                            <h4>TypeScript Errors</h4>
                            <p>Type checking and compilation errors</p>
                          </div>
                          <div className="category-count">{financialMetrics.debtStatus.summary.typescript}</div>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {/* Top Debt Hotspots - SSOT Data */}
                {(financialMetrics?.debtStatus?.topHotspots?.length || 0) > 0 && (
                  <div className="hotspots-section">
                    <h3>🔥 Top Debt Hotspots</h3>
                    <div className="hotspots-list">
                      {financialMetrics.debtStatus.topHotspots.slice(0, 10).map((hotspot, index) => (
                ...
                {/* Guido Messages if present - SSOT Data */}
                {financialMetrics?.debtStatus?.currentDebt?.Guido && financialMetrics.debtStatus.currentDebt.Guido.length > 0 && (
                  <div className="guido-messages-section">
                    <h3>🤌 Guido's Collection Notices</h3>
                    <div className="guido-messages">
                      {financialMetrics.debtStatus.currentDebt.Guido.map((message, index) => (
      ...
      {/* After Dark Mode Easter Egg - Removed for stability */}
